                ...
                if ($request->type == 'membership') {
                    ...
                    if (!empty(company_setting('New Membership Order', $membership->created_by, $membership->workspace)) && company_setting('New Membership Order', $membership->created_by, $membership->workspace) == true) {
                        ...
                        try {
                            ...
                            return redirect()->back()->with('success', __('Payment Pay Successfully.'));
                        } catch (\Exception $e) {
                            return redirect()->back()->with('success', __('Transaction has been failed.'));
                        }
                    }
                    return redirect()->back()->with('success', __('Payment Pay Successfully. email notification is off.'));
                } else if ($request->type == 'booking') {
                    ...
                    if (!empty(company_setting('New Booking Order', $booking->created_by, $booking->workspace)) && company_setting('New Booking Order', $booking->created_by, $booking->workspace) == true) {
                        $amenitiesList = $amenitiesList = collect($booking->amenities)->pluck('name')->implode(', ');
                        $uArr = [
                            "customer_name" =>   $booking->customer_name,
                            "amenities" => $amenitiesList,
                            "booking_duration" => $booking->booking_duration,
                            "start_end_date_time" => company_date_formate($booking->start_date_time, $booking->created_by, $booking->workspace) . ' To ' . company_date_formate($booking->end_date_time, $booking->created_by, $booking->workspace),
        ...
        try {
            ...
            try {
                ...
                if (isset($response['status']) && $response['status'] == 'COMPLETED') {
                    ...
                    $sportsGroundBooking->payment_type      =  __('Paypal');
            ...
            try {
                ...
                if (isset($response['status']) && $response['status'] == 'COMPLETED') {
                    ...
                    if ($plan->duration == 'daily') {
                        $plan_end = date('Y-m-d', strtotime(date('Y-m-d H:i:s') . ' + 1 days'));
                    }

                    if ($plan->duration == 'weekly') {
                        $plan_end = date('Y-m-d', strtotime(date('Y-m-d H:i:s') . ' + 7 days'));
                    }

                    if ($plan->duration == 'monthly') {
                        $plan_end = date('Y-m-d', strtotime(date('Y-m-d H:i:s') . ' + 30 days'));
                    }

                    if ($plan->duration == 'yearly') {
                        $plan_end = date('Y-m-d', strtotime(date('Y-m-d H:i:s') . ' + 365 days'));
                    }
                    ...
                    if ($request->user_type == 'new-user') {
                        ...
                        $membershipOrder->membership_expiry_date    = $plan_end;
                    ...
                    if (!empty(company_setting('Sports Ground And Club Membership Order', $groundAndClubMember->created_by, $groundAndClubMember->workspace)) && company_setting('Sports Ground And Club Membership Order', $groundAndClubMember->created_by, $groundAndClubMember->workspace) == true) {
                        ...
                        $uArr = [
                            'member_name'                   => $groundAndClubMember->name,
                            'membership_plan_name'          => isset($membershipplan->name) ? $membershipplan->name : '-',
                            'membership_plan_amount'        => currency_format_with_sym($request->plan_price, $groundAndClubMember->created_by, $groundAndClubMember->workspace),
                            'membership_plan_duration'      => $membershipplan->duration,
                            'membership_expiry_date'        => company_date_formate($groundAndClubMember->membership_expiry_date, $groundAndClubMember->created_by, $groundAndClubMember->workspace),
                            'membership_order_url'          => $responsURL,
                            'company_name'                  => $company->name,
                        ];
                        try {
                            $resp = EmailTemplate::sendEmailTemplate(
